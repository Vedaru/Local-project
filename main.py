                ...
                if m:
                    ...
                    try:
                        ...
                        if not task_desc:
                            ...
                            logger.info(
                                f"SUMMON_AGENT JSON 无 task 字段(keys={list(payload.keys())})，使用用户原始输入作为任务: {task_desc}"
                            )
                        ...
                        # 完整查询应该包含中文、空格或较长的描述
                            ...
                            if len(task_desc) < 20 and re.match(r"^[a-zA-Z0-9_]+$", task_desc):
                                logger.warning(
                                    f"SUMMON_AGENT JSON 中的 task 看起来是工具名称 '{task_desc}'，不是完整查询。使用用户原始输入: {cleaned_input}"
                                )
                                task_desc = cleaned_input
                    except Exception as e:
...
只输出确认语，不要输出任何其他内容。"""
                    ...
                    if self.agent and task_desc:
                        ...
                        try:
                            ...
                            logger.info(
                                f"ManusAgent.run_task completed — result(len)={len(agent_result) if isinstance(agent_result, str) else 'N/A'}"
                            )
                        except Exception as e:
                    ...
                    try:
                        # 判断任务是否成功
                        is_success = (
                            "success" in agent_result.lower() or "完成" in agent_result or "成功" in agent_result
                        )
                        is_failure = (
                            "failure" in agent_result.lower()
                            or "失败" in agent_result
                            or "异常" in agent_result
                            or "错误" in agent_result
                        )
...
只输出确认语，不要输出任何其他内容。"""
                        ...
                        if voice_reply and len(voice_reply) < 50:
                            ...
                            # 回复太长，使用默认
                            default_reply = (
                                "搞定啦~" if is_success else ("抱歉没成功呢..." if is_failure else "处理好了~")
                            )
                ...
                try:
                    ...
                    if parsed_agent and isinstance(parsed_agent, dict) and "tool" in parsed_agent:
                        ...
                        if self.agent:
                            ...
                            try:
                                is_success = (
                                    "success" in agent_result.lower()
                                    or "完成" in agent_result
                                    or "成功" in agent_result
                                )
                                is_failure = (
                                    "failure" in agent_result.lower()
                                    or "失败" in agent_result
...
class MainApplication:
    ...
    def setup(self):
        ...
        # 创建 Avatar 窗口
        self.avatar = AvatarWidget(width=400, height=600, x=100, y=100)

        # 初始化口型同步管理器（通过信号更新，保证线程安全）
        self.lip_sync_manager = LipSyncManager(update_callback=lambda v: self.signals.lip_sync_update.emit(v))

        # 初始化表情管理器
        self.expression_manager = ExpressionManager(
            expression_callback=self._change_expression, motion_callback=self._play_motion
        )
    ...
    def _on_speak_request(self, text: str):
        ...
        if self.voice_manager and self.avatar:
            try:
                # 生成临时 wav 文件路径
                wav_path = os.path.join(os.path.dirname(__file__), "data", "temp", f"tts_{int(time.time() * 1000)}.wav")
