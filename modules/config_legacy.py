...
from .config_base import GPT_SOVITS_ROOT, PROJECT_ROOT, _clean_env_value, _env_vars, get_yaml_config
...
_api_key = _clean_env_value(os.getenv("ARK_API_KEY") or _env_vars.get("ARK_API_KEY") or _api_cfg.get("ark_api_key"))
_base_url = (
    _clean_env_value(os.getenv("ARK_BASE_URL") or _env_vars.get("ARK_BASE_URL") or _api_cfg.get("ark_base_url"))
    or "https://ark.cn-beijing.volces.com/api/v3"
)
_client = None  # 延迟初始化
_async_client = None  # 延迟初始化（异步版）
...
def _get_async_client():
    """按需创建 AsyncOpenAI 客户端，避免导入时产生网络连接。"""
...
class _AsyncClientProxy:
    """透明代理：首次属性访问时才创建真正的 AsyncOpenAI 客户端。"""
...
client = _ClientProxy()
async_client = _AsyncClientProxy()


# ---- 音频/TTS 配置常量 ----
SOVITS_URL: str = _clean_env_value(os.getenv("SOVITS_URL") or _api_cfg.get("sovits_url")) or "http://127.0.0.1:9880"
...
PROMPT_TEXT: str = str(_audio_cfg.get("prompt_text", "大家好，我是虚拟歌手洛天依，欢迎来到我的十周年生日会直播。"))
...
MODEL_NAME: Optional[str] = _clean_env_value(
    os.getenv("MODEL_NAME") or _env_vars.get("MODEL_NAME") or _api_cfg.get("model_name")
)

# ---- 嵌入模型名称（可选）----
EMBEDDING_MODEL_NAME: Optional[str] = _clean_env_value(
    os.getenv("EMBEDDING_MODEL_NAME") or _env_vars.get("EMBEDDING_MODEL_NAME") or _api_cfg.get("embedding_model_name")
)
