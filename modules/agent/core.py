...
LLM 配置统一：
    ...
    API 设置(model, base_url, api_key)默认从项目根目录 project_config.yaml 读取，
    并支持环境变量覆盖，无需手动编辑 config.toml。
"""
...
def _sync_openmanus_config():
    """将项目的 LLM API 设置同步写入 OpenManus 的 config.toml。
        ...
        核心思路：项目根目录 project_config.yaml 是唯一配置源，OpenManus 的
        config.toml 是派生文件，每次 ManusAgent 初始化时自动重新生成。
        环境变量仅作为运行时覆盖层。

    读取来源：
            - openmanus.* / api.* / tuning.* → project_config.yaml
            - ARK_API_KEY / ARK_BASE_URL / MODEL_NAME → 环境变量覆盖
    """
    ...
    with _config_sync_lock:
        ...
        from ..config import get_cached_config, get_yaml_config

        cfg = get_cached_config()
        yaml_cfg = get_yaml_config()

        openmanus_cfg = yaml_cfg.get("openmanus", {}) if isinstance(yaml_cfg, dict) else {}
        llm_cfg = openmanus_cfg.get("llm", {}) if isinstance(openmanus_cfg, dict) else {}
        search_cfg = openmanus_cfg.get("search", {}) if isinstance(openmanus_cfg, dict) else {}
        browser_cfg = openmanus_cfg.get("browser", {}) if isinstance(openmanus_cfg, dict) else {}
        mcp_cfg = openmanus_cfg.get("mcp", {}) if isinstance(openmanus_cfg, dict) else {}

        def _escape_toml_string(value: object) -> str:
            text = str(value or "")
        ...
        api_version = str(llm_cfg.get("api_version", "") or "")

        search_engine = str(search_cfg.get("engine", "Baidu") or "Baidu")
        fallback_engines_raw = search_cfg.get("fallback_engines", ["DuckDuckGo", "Bing"])
        if isinstance(fallback_engines_raw, list):
            fallback_engines = [str(item) for item in fallback_engines_raw if str(item or "").strip()]
        else:
            fallback_engines = ["DuckDuckGo", "Bing"]
        if not fallback_engines:
            fallback_engines = ["DuckDuckGo", "Bing"]
        fallback_engines_toml = "[" + ", ".join(f'"{_escape_toml_string(name)}"' for name in fallback_engines) + "]"
        search_lang = str(search_cfg.get("lang", "zh") or "zh")
        search_country = str(search_cfg.get("country", "cn") or "cn")

        browser_headless = bool(browser_cfg.get("headless", False))
        browser_disable_security = bool(browser_cfg.get("disable_security", True))

        mcp_server_reference = str(mcp_cfg.get("server_reference", "app.mcp.server") or "app.mcp.server")
...
# ║  修改配置请编辑项目根目录的 project_config.yaml              ║
# ╚══════════════════════════════════════════════════════════════╝
...
api_type = "{_escape_toml_string(api_type)}"
api_version = "{_escape_toml_string(api_version)}"

[search]
engine = "{_escape_toml_string(search_engine)}"
fallback_engines = {fallback_engines_toml}
lang = "{_escape_toml_string(search_lang)}"
country = "{_escape_toml_string(search_country)}"
...
[mcp]
server_reference = "{_escape_toml_string(mcp_server_reference)}"
"""
...
class ManusAgent:
    ...
    LLM 设置自动从项目 root 配置读取（支持环境变量覆盖），无需额外配置。
