...
call :check_exists "install_dependencies.bat 存在" "%PROJECT_DIR%install_dependencies.bat" "缺少安装脚本"
...
if "%PASSED%"=="%TOTAL%" (
    ...
    echo 运行: .\run_with_runtime.bat
) else if %RATE% GEQ 70 (
    echo.
    echo [WARN] Runtime 环境部分配置有问题，建议修复后使用
    echo 如果缺少依赖，运行: .\install_dependencies.bat
) else (
    echo.
...
if errorlevel 1 (
    echo [✗] %~1
    echo     提示: 运行 install_dependencies.bat 安装依赖
) else (
    set /a PASSED+=1
    echo [✓] %~1
)
