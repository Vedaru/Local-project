...
class MainApplication:
    ...
    def setup(self):
        ...
        if self.sovits_process is None:
            print("警告: GPT-SoVITS API 服务启动失败。")
    ...
    def _on_speak_request(self, text: str):
        ...
        print(f"[TTS] 收到语音请求: {text[:50]}...")  # 调试输出
        ...
        if self.voice_manager and self.avatar:
            try:
                ...
                print(f"[TTS] wav 保存路径: {wav_path}")  # 调试输出
                
                # 在子线程中执行 TTS（避免阻塞主线程）
                def speak_with_browser():
                    try:
                        print("[TTS] 开始合成语音...")  # 调试输出
                        ...
                        print(f"[TTS] 语音合成成功, 文件存在: {os.path.exists(wav_path)}")  # 调试输出
                        
                        # 2. 通过信号让主线程播放音频
                        print("[TTS] 发送 play_audio 信号...")  # 调试输出
                        ...
                        try:
                            ...
                            print(f"[TTS] 音频时长: {duration:.2f}秒")  # 调试输出
                                
                            # 等待播放完成后清理
                            time.sleep(duration + 0.5)
                            try:
                                os.remove(wav_path)
                                print("[TTS] 临时文件已清理")  # 调试输出
                            except:
                                pass
                        except Exception as e:
                            print(f"[TTS] 读取 wav 错误: {e}")  # 调试输出
            ...
            print(f"[TTS] voice_manager={self.voice_manager}, avatar={self.avatar}")  # 调试输出
    
    def _on_play_audio(self, wav_path: str):
        """在主线程中播放音频（由信号触发）"""
        print(f"[TTS] 主线程收到播放请求: {wav_path}")  # 调试输出
        if self.avatar:
    ...
    def run(self):
        ...
        # 显示启动信息
        stats = self.memory_manager.get_memory_stats()
        print("\nProject Local 已启动（带 Avatar）。")
        print(f"记忆状态: 短期({stats['short_term']}/{stats['short_term_capacity']}) | "
              f"长期({stats['long_term']}) | 情感({stats['emotional']})")
        print("输入 'exit' 或 'quit' 退出，输入 'status' 查看记忆状态。\n")
